...
class Custom_template { 
	...
	public function getRouteData($route):array {
		if (!isset($this->custom_template_data[$route])) {
			...
			if ($custom_template_data->num_rows) {
				...
				$this->custom_template_data[$route] = [];
			}
		}
		return $this->custom_template_data[$route];
	}
	...
	public function getTotal($currency):int {
	...
	public function getSubTotal($currency):int {
	...
	public function getWeight($weight_class_id):int {
	...
	public static function withinRange($value, $min, $max, $inclusive = true):bool {
		if ($min == 0 && $max == 0) {
			return true;
		} elseif ($min == 0) {
	...
	public function getCustom($id):string {
	...
	public function filterTemplate($route):mixed {
		$this->getRouteData($route);
		if (isset($this->custom_template_data[$route])) {
			foreach ($this->custom_template_data[$route] as $key => $filter_data) {
				$template = $filter_data['template'];
				if ($this->filter($filter_data)) {
					return $template;
				}
			}
		}
	...
	public function filter($filter_data):bool {
		$result = true;
