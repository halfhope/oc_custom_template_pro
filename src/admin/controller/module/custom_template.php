...
class CustomTemplate extends \Opencart\System\Engine\Controller {
	...
	public function index():void {
		...
		$data['action'] = $this->url->link($this->_route . '|save', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		foreach ($results as $key => $value) {
			...
			$explain = $this->getExplain($custom_template);
	...
	public function save():void {
		...
		if ($this->validate()) {
			...
			$json['error']['error_warning'] = $this->error['warning'];
		}
	...
	private function getExplain(array $settings = []):array {
		$this->loadDataModels();
		$this->load->language($this->_route);
		...
		if (isset($settings['template'])) {
			if ($settings['template']) {
				$response[] = sprintf($this->language->get('text_output_template'), '<span class="badge bg-primary">' . $settings['template'] . '</span>');
			}
		}
		...
		if (isset($settings['platform'])) {
			if ($settings['platform']) {
				...
				$response[] = '<span class="badge bg-secondary">' . implode(', ', $settings['platform']) . '</span>';
			}
		}
		...
		if (isset($settings['browser'])) {
			if ($settings['browser']) {
				...
				$response[] = '<span class="badge bg-secondary">' . implode(', ', $settings['browser']) . '</span>';
			}
		}
		...
		if (count($response) == 1) {
			...
			$json['description'] .= '<span class="badge bg-warning">' . $this->language->get('text_output_notset') . '</span>';
		}
	...
	public function explain():void {
		$explain = $this->getExplain($this->request->post);
		$this->response->addHeader('Content-type:application/json;charset=utf-8');
		$this->response->setOutput(json_encode($explain));
	}
