	...
	if (sd || overwrite) {
		$.ajaxQueue({
			...
			success: function(response) {
				$('#ct-row-' + module_row).find('td.description').html('<span class="btn-sm sort-handle"><i class="fa fa-arrows-alt-v"></i></span>' + response.description);
			},
			error: function(err) {
...
var addCtGroup = function(route) {
	...
	if ($ct_group.length) {
		...
		html += '			<button data-bs-toggle="tooltip" title="' + ct_data.lang.button_add + '" class="btn btn-success btn-sm add" data-route="' + route + '"><i class="fa fa-plus"></i></button>';
		html += '		</div>';
		html += '	</div>';
...
var addCtGroupItem = function($ct_group) {
	...
	html += '		<span class="btn-sm sort-handle"><i class="fa fa-arrows-alt-v"></i></span><span class="badge bg-warning">' + ct_data.lang.text_output_notset + '</span>';
	html += '	</td>';
	html += '	<td width="80">';
	html += '		<button data-bs-toggle="tooltip" class="btn btn-primary btn-sm edit" title="' + ct_data.lang.text_edit + '" data-module-row="' + module_row + '"><i class="fa fa-pencil-alt"></i></button>';
	html += '		<button data-bs-toggle="tooltip" class="btn btn-danger btn-sm remove" title="' + ct_data.lang.button_remove + '"><i class="fa fa-trash"></i></button>';
...
$(document).ready(function() {
	// init popovers
	$('.ct-group a[data-bs-toggle=popover]').popover();
	...
	$('#mmct-modal button.save').on('click', function(event) {
		...
		$('#ct-row-' + module_row + ' td:last-child').append($module_row_node);
