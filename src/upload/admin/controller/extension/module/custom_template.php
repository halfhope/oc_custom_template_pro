...
class ControllerExtensionModuleCustomTemplate extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			foreach($events as $event) {
				if(!$result = $this->model_extension_event->getEvent($event['code'], $event['trigger'], $this->_route . $event['action'])) {
					$this->model_extension_event->addEvent($event['code'], $event['trigger'], $this->_route . $event['action']);
				}
			}
		...
		$data['action'] = $this->url->link($this->_route, 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');
		
		$data['form'] = htmlspecialchars_decode($this->url->link($this->_route . '/form', 'token=' . $this->session->data['token'], 'SSL'));
		$data['explain'] = htmlspecialchars_decode($this->url->link($this->_route . '/explain', 'token=' . $this->session->data['token'], 'SSL'));
		$data['find_mods'] = htmlspecialchars_decode($this->url->link($this->_route . '/find_mods', 'token=' . $this->session->data['token'], 'SSL'));
		$data['product_autocomplete'] = htmlspecialchars_decode($this->url->link($this->_route . '/product_autocomplete', 'token=' . $this->session->data['token'], 'SSL'));
	...
	public function find_mods($route = false) {
		...
		foreach ($modified_files as $key => $value) {
			if (substr_count($key, $group_route . '.tpl') > 0) {
				$response = $value;
			}
		}
